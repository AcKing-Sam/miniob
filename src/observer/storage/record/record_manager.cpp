...
RC PaxRecordPageHandler::insert_record(const char *data, RID *rid)
{
  ...
  // insert data here, assert index < page_header_->record_capacity
  // partition the record to several cols
  // then, insert the record's each col into page
  int idx = 0;
  int prev_cols_len = 0;
  ...
  for(int i = 0;i < chunk.column_num();i ++) {
    ...
    if(idx == 0) {
      ...
      for(int j = 0;j < page_header_->record_num;j ++) {
        if(bitmap.get_bit(j)) {
          col->append_one(frame_->data() + page_header_->data_offset + next_off);
          next_off += col_len;
        }
      }
    } else {
      ...
      for(int j = 0;j < page_header_->record_num;j ++) {
        if(bitmap.get_bit(j)) {
          col->append_one(frame_->data() + page_header_->data_offset + column_index[idx - 1] + next_off);
          next_off += col_len;
        }
      }
    } 
